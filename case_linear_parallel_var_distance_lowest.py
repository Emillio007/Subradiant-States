...
"""
Different inter-atomic distances in linear parallel one-dim case.
"""
lowa, higha = 0.1, 0.4
...
ax1.set_title("\n".join(wrap(r"Twin axes lowest decay rate vs. $\frac{d}{\lambda_0} \in [0.1, 0.4]$ for $N=%s$ and vs. $N \in [40, 60]$ for $\frac{d}{\lambda_0}=%s$ in linear chain of parallel polarized dipoles" % (N, a_static), 80)), loc="center")
plt.savefig("figures/case_linear_parallel_var_distance_01_04_var_N_40_60_lowest.png", dpi=300)
p.show()